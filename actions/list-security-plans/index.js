...
module.exports = async ({ category }) => {
  try {
    ...
    // Extract device count from description where available
    const plansWithDevices = filteredPlans.map(plan => {
      const deviceMatch = plan.description.match(/up to (\d+) devices?/i) ||
                          plan.description.match(/(\d+) (?:Windows|macOS|Android|iOS) devices?/i);
      return {
        ...plan,
        devices: deviceMatch ? `Up to ${deviceMatch[1]} devices` : 'Multiple devices supported'
      };
    });

    const summary = category
      ? `Found ${filteredPlans.length} ${category} plan${filteredPlans.length !== 1 ? 's' : ''}.`
      : `Found ${PLANS.length} security plans across all categories.`;

    return {
      content: [
        { type: 'text', text: summary }
      ],
      structuredContent: plansWithDevices
    };
  } catch (error) {
    return {
      content: [
        { type: 'text', text: `Error retrieving security plans: ${error.message}` }
      ],
      structuredContent: []
    };
  }
