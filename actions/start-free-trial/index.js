...
module.exports = async ({ plan_name, email }) => {
  try {
    ...
    if (!emailRegex.test(email)) {
      return {
        content: [
          { type: 'text', text: 'Invalid email address format. Please provide a valid email.' }
        ],
        structuredContent: {
          status: 'failed',
          trial_plan: null,
    ...
    if (!plan) {
      return {
        ...
        structuredContent: {
          ...
          trial_plan: null,
          trial_duration: null,
          message: `Plan "${plan_name}" not found.`
        }
      };
    }
    ...
    const activationResult = {
      ...
      message: `Your 30-day free trial of ${plan.name} has been activated! Check your email (${email}) for download instructions and activation details. No credit card required.`
    };

    return {
      content: [
        { type: 'text', text: `Free trial activated for ${plan.name}. Check ${email} for instructions.` }
      ],
      structuredContent: activationResult
    };
  } catch (error) {
    return {
      content: [
        { type: 'text', text: `Error starting free trial: ${error.message}` }
      ],
      structuredContent: {
        status: 'failed',
        trial_plan: null,
